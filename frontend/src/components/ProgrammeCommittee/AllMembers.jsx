...
const AllProgrammeCommitteeMembers = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="admin-title">Programme Committee</h2>
        <p className="admin-muted mt-1">Manage programme committee members.</p>
      </div>
          ...
          {members.map((member) => (
            <div key={member.id} className="admin-card overflow-hidden">
              ...
              <div className="p-4">
                ...
                <p className="text-zinc-600 mb-4 text-sm line-clamp-2">{member.specialization}</p>
                <p className="text-zinc-700 mb-4 text-sm">
                  <b>{member.college}</b>
                </p>

                <div className="flex justify-between items-center">
                  <button onClick={() => handleDelete(member.id)} className="admin-button-danger">
                    Delete
                  </button>
                  <button
                    onClick={() => navigate(`/admin/all-programme-members/${member.id}`)}
...
export default AllProgrammeCommitteeMembers;
