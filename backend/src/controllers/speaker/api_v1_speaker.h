...
class speaker : public drogon::HttpController<speaker>
{
  public:
    ...
    METHOD_ADD(speaker::deleteSpeaker, "/{1}", Delete,"AuthFilter"); // path -> /api/v1/speaker/{speakerId}
    METHOD_ADD(speaker::updateSpeaker, "/{1}", Put,"AuthFilter"); // path -> /api/v1/speaker/{speakerId}
    METHOD_ADD(speaker::createSpeaker, "", Post,"AuthFilter"); // path -> /api/v1/speaker
